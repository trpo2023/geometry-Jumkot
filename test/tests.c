...
CTEST(calculate, perimeter_count)
{
    ...
    double result = perimeter_count(&Circle);
    ASSERT_DBL_NEAR_TOL(expect, result, tol);

    Circle.radius = 2.75;
    expect = 17.27876;
    result = perimeter_count(&Circle);
    ASSERT_DBL_NEAR_TOL(expect, result, tol);
...
CTEST(calculate, area_count)
{
    ...
    double result = area_count(&Circle);
    ASSERT_DBL_NEAR_TOL(expect, result, tol);

    Circle.radius = 2.75;
    expect = 23.758294;
    result = area_count(&Circle);
    ASSERT_DBL_NEAR_TOL(expect, result, tol);
...
CTEST(parser, instersect)
{
    ...
    bool expected = true;
    bool result = intersect(&Circle_1, &Circle_2);
    ASSERT_EQUAL(expected, result);
    ...
    expected = false;
    result = intersect(&Circle_1, &Circle_2);
    ASSERT_EQUAL(expected, result);
}